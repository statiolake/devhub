# Run one authenticated Workbench origin

DevHub runs one Workbench server process on `127.0.0.1` and gives every Editor Surface the same stable origin. The first successful launch selects and persists a port; later launches fail visibly rather than silently changing origin when that port is occupied.

Authentication uses a generated 32-byte token stored in an owner-readable token file under `Application Support/DevHub/VisualStudioCode`. The server receives only the token-file path. Server data, extensions, and logs remain in the same application-owned directory. Telemetry and experiments are disabled while Workspace Trust remains upstream behavior.
...
- Extension availability follows the installed VS Code; DevHub adds only its own Bridge VSIX.
