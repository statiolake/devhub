...
- **The fix for looking up a container or SSH Agent's command on the
  Workspace's own machine has not been checked in a real container yet.**
  Before it, a profile command was looked up on this Mac, so the container
  check above had to give the fake agent a path that existed on both sides.
  Since 59c9eb8 the lookup runs `command -v` on the Workspace's machine
  (`Runtime.resolveProgram`), and it is tested against a container runtime.
  No packaged build with that change has been run against a container yet.
