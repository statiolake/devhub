...
- **A container or SSH Agent's command is looked up on this Mac.** Until that
  is fixed, a profile whose `command` exists only in the container or on the
  remote host is refused before launch. A bare name such as `claude` is
  resolved to this Mac's absolute path, which must then also exist at the same
