...
// Without the proposal in the packaged manifest the resolver registration is
// refused at runtime, and the window it was opened for never resolves.
if (!packageJson.enabledApiProposals?.includes("resolvers")) {
  throw new Error("VSIX manifest must enable the resolvers API proposal");
}
const staging = await mkdtemp(resolve(tmpdir(), "devhub-remote-vsix-"));
...
await writeFile(
  ...
  `<?xml version="1.0" encoding="utf-8"?>\n<PackageManifest Version="2.0.0" xmlns="http://schemas.microsoft.com/developer/vsx-schema/2011" xmlns:d="http://schemas.microsoft.com/developer/vsx-schema/2011/d"><Metadata><Identity Language="en" Id="devhub.remote" Version="${packageJson.version}" Publisher="${packageJson.publisher}"/><DisplayName>${packageJson.displayName}</DisplayName><Description xml:space="preserve">${packageJson.description}</Description><Tags>devhub,remote,ssh</Tags><Categories>Other</Categories><GalleryFlags>Public</GalleryFlags><Properties><Property Id="Microsoft.VisualStudio.Code.Engine" Value="${packageJson.engines.vscode}"/></Properties></Metadata><Installation><InstallationTarget Id="Microsoft.VisualStudio.Code" Version="[1.109.0,2.0.0)"/></Installation><Dependencies/><Assets><Asset Type="Microsoft.VisualStudio.Code.Manifest" Path="extension/package.json"/><Asset Type="Microsoft.VisualStudio.Services.Content.Details" Path="extension/README.md"/></Assets></PackageManifest>\n`,
);
...
console.log(`DevHub Remote package written: ${output}`);
