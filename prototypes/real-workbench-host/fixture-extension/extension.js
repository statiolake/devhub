...
function workspaceRoot() {
  const folders = vscode.workspace.workspaceFolders;
  return folders && folders.length > 0 ? absolutePath(folders[0].uri) : null;
}

// A real OpenVSCode server gives each remote extension host the same process
// environment. Derive the Bridge identity from the public workspace context
// so two child Workbenches can be observed independently without injecting
// per-child JavaScript or forking OpenVSCode.
...
function realIdentity() {
  ...
  const identitySeed = root ? `workspace:${root}` : 'global';
  return {
    root,
    surfaceId: stableUuid(`surface:${identitySeed}`),
    workspaceId: stableUuid(`workspace:${identitySeed}`),
  };
}
...
function makeBridgeExtension(context) {
  ...
  const realFixture = process.env.DEVHUB_REAL_WORKBENCH_FIXTURE === '1';
  const identity = realFixture ? realIdentity() : null;
  const surfaceId = realFixture
    ? identity.surfaceId
    : process.env.DEVHUB_BRIDGE_SURFACE_ID;
  const workspaceId = realFixture
    ? identity.workspaceId
    : process.env.DEVHUB_BRIDGE_WORKSPACE_ID || crypto.randomUUID();
  ...
  const hotExitPhase = process.env.DEVHUB_REAL_HOT_EXIT_PHASE || 'off';
  const hotExitRoot = process.env.DEVHUB_REAL_HOT_EXIT_ROOT || '';
  const hotExitMarkerName = process.env.DEVHUB_REAL_HOT_EXIT_MARKER || '.devhub-f02-hot-exit.json';
  const hotExitFileName = process.env.DEVHUB_REAL_HOT_EXIT_FILE || '.devhub-f02-hot-exit.txt';
  const hotExitPreparedKey = process.env.DEVHUB_REAL_HOT_EXIT_PREPARED_KEY || 'devhub.real.hotExitPrepared';
  const hotExitRestoreRequestName = process.env.DEVHUB_REAL_HOT_EXIT_RESTORE_REQUEST || '.devhub-f02-restore-request';
  const storageKey = process.env.DEVHUB_REAL_STORAGE_KEY || 'devhub.real.storage.key';
  const storageValue = process.env.DEVHUB_REAL_STORAGE_VALUE || 'unset';
  const storageWriterRoot = process.env.DEVHUB_REAL_STORAGE_WRITER_ROOT || '';
  const storageReaderRoot = process.env.DEVHUB_REAL_STORAGE_READER_ROOT || '';
  const storageRunId = process.env.DEVHUB_REAL_STORAGE_RUN_ID || 'unidentified';
  const storageMarkerName = process.env.DEVHUB_REAL_STORAGE_MARKER || '.devhub-f02-storage.json';
  ...
  if (realFixture) {
    log('real_fixture_identity', {
      surface_id: surfaceId,
      workspace_id: workspaceId,
      canonical_root: identity.root,
      hot_exit_phase: hotExitPhase,
      storage_role: identity.root === storageWriterRoot
        ? 'writer'
        : identity.root === storageReaderRoot ? 'reader' : 'none',
    });
  }
    ...
    socket = new LoopbackWebSocket(endpoint, token, {
      onOpen: () => {
        ...
        const hello = envelope('hello', {
          ...
          surface_id: surfaceId,
          workbench_instance_id: context.extensionMode === vscode.ExtensionMode.Development ? crypto.randomUUID() : crypto.randomUUID(),
        });
  ...
  async function dirtyAutomation() {
    await waitReady();
    if (realFixture && (hotExitPhase === 'prepare' || hotExitPhase === 'auto') && identity.root === hotExitRoot) {
      // Use a file-backed document for the native close/relaunch proof. The
      // setting is the public VS Code hot-exit contract; no browser state is
      // read or injected.
      ...
      const uri = vscode.Uri.joinPath(vscode.workspace.workspaceFolders[0].uri, hotExitFileName);
      await vscode.workspace.fs.writeFile(uri, Buffer.from('F0.2 hot-exit baseline\n'));
      const document = await vscode.workspace.openTextDocument(uri);
      const editor = await vscode.window.showTextDocument(document);
      await editor.edit((builder) => builder.insert(new vscode.Position(0, 0), 'unsaved-real-edit\n'));
      emitDirtyIfChanged();
      log('dirty_automation_complete', {
        fixture: 'file-backed-document',
        dirty: true,
        uri_scheme: uri.scheme,
        api: 'vscode.workspace.fs+openTextDocument+TextEditor.edit',
      });
      return;
    }
    ...
    // public API state is observed before and after the extension-host reload.
    log('dirty_automation_complete', { fixture: 'two-untitled-content-documents', dirty: true });
  }

  async function storageAutomation() {
    if (!realFixture || !identity.root) return;
    if (identity.root === storageWriterRoot) {
      // Both stores are written through the public VS Code extension API. The
      // global key is intentionally shared by the profile; workspaceState is
      // intentionally scoped to this folder's storage bucket.
      await context.globalState.update(storageKey, storageValue);
      await context.workspaceState.update(storageKey, `workspace-only:${storageValue}`);
      await vscode.workspace.fs.writeFile(
        vscode.Uri.joinPath(vscode.workspace.workspaceFolders[0].uri, storageMarkerName),
        Buffer.from(JSON.stringify({
          role: 'writer',
          run_id: storageRunId,
          global_value: storageValue,
          workspace_value: `workspace-only:${storageValue}`,
        })),
      );
      log('storage_write', {
        run_id: storageRunId,
        ...
        global_value: storageValue,
        workspace_value: `workspace-only:${storageValue}`,
        api: 'ExtensionContext.globalState+workspaceState',
      });
      return;
    }
    if (identity.root !== storageReaderRoot) return;

    const deadline = Date.now() + 15000;
    let globalValue;
    while (Date.now() < deadline) {
      globalValue = context.globalState.get(storageKey);
      if (globalValue === storageValue) break;
      await new Promise((resolve) => setTimeout(resolve, 250));
    }
    const workspaceValue = context.workspaceState.get(storageKey);
    await vscode.workspace.fs.writeFile(
      vscode.Uri.joinPath(vscode.workspace.workspaceFolders[0].uri, storageMarkerName),
      Buffer.from(JSON.stringify({
        role: 'reader',
        run_id: storageRunId,
        global_value: globalValue === undefined ? null : globalValue,
        global_match: globalValue === storageValue,
        workspace_value_present: workspaceValue !== undefined,
        workspace_value: workspaceValue === undefined ? null : workspaceValue,
      })),
    );
    log('storage_read', {
      run_id: storageRunId,
  ...
  async function hotExitAutomation() {
    if (!realFixture || hotExitPhase === 'off' || identity.root !== hotExitRoot) return;
    await waitReady();
    const prepared = context.globalState.get(hotExitPreparedKey, false);
    const restoreRequest = vscode.Uri.joinPath(
      vscode.workspace.workspaceFolders[0].uri,
      hotExitRestoreRequestName,
    );
    let restoreRequested = false;
    try {
      await vscode.workspace.fs.stat(restoreRequest);
      restoreRequested = true;
    } catch { /* the native host has not completed the close phase */ }
    if (hotExitPhase === 'prepare' || (hotExitPhase === 'auto' && !prepared && !restoreRequested)) {
      await context.globalState.update(hotExitPreparedKey, true);
      await dirtyAutomation();
      log('hot_exit_prepare_complete', {
        canonical_root: identity.root,
        dirty_documents: vscode.workspace.textDocuments.filter((document) => document.isDirty).length,
      });
      return;
    }
    if (hotExitPhase === 'auto' && !restoreRequested) return;
    if (hotExitPhase !== 'restore' && hotExitPhase !== 'auto') return;

    // Workbench hot exit restores untitled backups before the extension host
    // reaches this callback. Observe only the public document model.
    await new Promise((resolve) => setTimeout(resolve, 1000));
    const dirtyDocuments = vscode.workspace.textDocuments.filter((document) => document.isDirty);
    emitDirtyIfChanged();
    await vscode.workspace.fs.writeFile(
      vscode.Uri.joinPath(vscode.workspace.workspaceFolders[0].uri, hotExitMarkerName),
      Buffer.from(JSON.stringify({
        phase: 'restore',
        dirty: dirtyDocuments.length > 0,
        dirty_documents: dirtyDocuments.length,
        schemes: dirtyDocuments.map((document) => document.uri.scheme),
      })),
    );
    log('hot_exit_restore_observed', {
      canonical_root: identity.root,
      dirty: dirtyDocuments.length > 0,
      dirty_documents: dirtyDocuments.length,
      schemes: dirtyDocuments.map((document) => document.uri.scheme),
      api: 'vscode.workspace.textDocuments/document.isDirty',
    });
  }
  ...
  async function boundaryAutomation() {
    ...
    // the public command without awaiting it so the second request and the
    // bounded reload can still be exercised in the same Workbench session.
  ...
  if (realFixture) {
    Promise.resolve().then(async () => {
      try {
        await storageAutomation();
        await hotExitAutomation();
      } catch (error) {
        log('real_fixture_automation_error', { summary: String(error.message).slice(0, 256) });
      }
    });
  } else if (automation) {
    ...
    if (stage === 0) {
      context.globalState.update('devhub.bridge.automation.stage', 1).then(async () => {
        try {
          ...
          await dirtyAutomation();
          // Give the public openFolder command time to cross the host URL
          // boundary before issuing the reload that proves restart.
          await new Promise((resolve) => setTimeout(resolve, 3000));
          log('requesting_workbench_reload');
          // Reload is a public Workbench command. The isolated host harness
          // allows this one controlled same-URL navigation, after which the
          // extension host must reconnect and restore the dirty fixture.
      ...
      sendSnapshot('ready');
      context.globalState.update('devhub.bridge.automation.stage', 2);
    }
  }
}
