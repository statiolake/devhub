...
/**
 * How loud an unread Agent is.
 *
 * The owner's report was that the mark "barely lights up" — a 6px dot alone in
 * a 14px rail, and nothing at all on the mark beside it. What answers that is
 * a size and two strengths, and the point of asserting them here is that there
 * are only ever those three: an unread row is loud in one place, and a later
 * change to how loud has one value to move rather than five rules to find.
 *
 * The same reason as the block above: these are custom properties in a
 * stylesheet, and jsdom resolves neither `color-mix` nor a custom property, so
 * there is nothing on a rendered node to measure.
 */
describe("how loud an unread Agent's mark is", () => {
  const shell = readFileSync("src/shell/styles/shell.css", "utf8");
  const tokens = readFileSync("src/shell/styles/tokens.css", "utf8");

  it("is a size and two strengths, each named once", () => {
    expect(tokens).toContain("  --unread-dot: 8px;");
    expect(tokens).toContain("  --unread-halo-spread: 2px;");
    expect(tokens).toContain("  --unread-halo: 40%;");
    // Waiting is the one status that is about the person, so it stays the
    // loudest of the five when it is also unread.
    expect(tokens).toContain("  --unread-halo-waiting: 55%;");
  });

  it("firms up with everything else under Increase Contrast", () => {
    // The strengths move, the colours do not — which is the whole reason they
    // are strengths. A mark a person is scanning the pane for must not be the
    // one soft thing left on the screen when the rest has hardened.
    expect(tokens).toContain(
      "    --unread-halo: 62%;\n    --unread-halo-waiting: 80%;",
    );
  });

  it("draws the dot at the size, and takes no size of its own", () => {
    expect(shell).toContain(
      "  width: var(--unread-dot);\n  height: var(--unread-dot);",
    );
    // The 6px this replaced, gone from the file rather than left beside its
    // replacement.
    expect(shell).not.toContain(".row-unread {\n  flex: none;\n  width: 6px;");
  });

  it("is one halo, on both of the row's marks", () => {
    // The dot says the Agent asked and the status mark says what it is doing;
    // both are lit, by one rule, so the second is recognised from the first.
    expect(shell).toContain(
      ".row-unread::before,\n.agent-row.is-unread .status-mark::before {\n  position: absolute;\n  border: var(--unread-halo-spread) solid\n    color-mix(in srgb, currentcolor var(--unread-halo), transparent);",
    );
    // Out of flow, so no row grows and the tree's elbow keeps its clearance.
    expect(shell).toContain("  inset: calc(-1 * var(--unread-halo-spread));");
  });

  it("says which colour each reason is, and invents none", () => {
    // The ink is per status; the two strengths it is used at are not. Every
    // mark paints itself out of `currentcolor`, so a status names its colour
    // once and nothing here can drift from the status marks themselves.
    ...
    expect(shell).toContain(
      ".agent-row.is-unread .status-mark-waiting,\n.row-unread-waiting {\n  --unread-halo: var(--unread-halo-waiting);\n}",
    );
  });
});
