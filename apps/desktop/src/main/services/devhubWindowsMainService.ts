...
const upstreamOpenInBrowserWindow = (WindowsMainService.prototype as unknown as WindowsMainServiceInternals).openInBrowserWindow;

(DevHubWindowsMainService.prototype as unknown as WindowsMainServiceInternals).openInBrowserWindow = async function (this: WindowsMainServiceInternals, options) {
	const workspace = options.workspace;
	const folder = isSingleFolderWorkspaceIdentifier(workspace) && workspace.uri.scheme === Schemas.file
	...
	const existing = existingId === undefined ? undefined : this.getWindowById(existingId);
	if (existing) {
		console.log(`[devhub] open: '${entry.name}' already has a view — showing it`);
		await controller.select(entry.id);
		return existing;
	}

	console.log(`[devhub] open: '${entry.name}' is new — a workbench view in the shell`);
