/**
 * Editing the person's last message, end to end on this machine: the real
 * host, the fake agent playing a Claude that is started again at the cut
 * (`claude-edit.*.ndjson`), the real Claude adapter and conversation — and a
...
describe("editing the last message through a real host", () => {
	const FIXTURES = fileURLToPath(
		new URL("../../../../test/fixtures/agent-conversation/", import.meta.url),
	);

	it("starts the fake Claude again at the cut, draws the edit, and a new DevHub replays to the same", async () => {
		...
		const host = spawn(command.file, [...command.args], {
			...
			env: {
				...process.env,
				FAKE_AGENT_SCRIPT: join(FIXTURES, "claude-edit.first.ndjson"),
				FAKE_AGENT_REWIND_SCRIPT: join(FIXTURES, "claude-edit.rewound.ndjson"),
			},
		});
		hosts.push(host);
		...
		const turnsEnded = (count: number) => (transcript: Transcript) =>
			...
			transcript.entries.filter((entry) => entry.kind === "turn-end").length ===
				count;
		for (const [index, text] of ["Say one", "Say two"].entries()) {
			await conversation.command({ kind: "send", text, origin: "person" });
			await until(conversation, turnsEnded(index + 1));
		}
		const last = editableMessage(conversation.reading().transcript);
		expect(last?.text).toBe("Say two");

		expect(await conversation.editLastMessage(last!.id, "Say three")).toBe(
			"sent",
		);
