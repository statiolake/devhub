...
export function appearanceWire(
  ...
  const wire: AppAppearanceWire = {
    ...
    titleBar: config.titleBar === "hidden" ? "hidden" : "shown",
 ...
 * two different things: at launch there is nothing but the defaults, and
 * later the last settings accepted stay in effect.
 */
export function settingsRefused(
  file: string,
  diagnostic: ConfigDiagnostic,
  runningOn: "the defaults" | "the last accepted settings",
): AppErrorWire {
  ...
  const instead =
    runningOn === "the defaults"
      ? "DevHub is running on its defaults until the file is fixed."
