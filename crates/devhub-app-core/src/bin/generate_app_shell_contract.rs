...
fn format_artifact(root: &Path, path: &Path, contents: String) -> String {
    ...
    match child.wait_with_output() {
        Ok(output) if output.status.success() => {
            String::from_utf8(output.stdout).unwrap_or(contents)
        }
        _ => contents,
    }
}
...
fn main() {
    ...
    for (path, raw_contents) in outputs(root) {
        let contents = format_artifact(root, &path, raw_contents);
        if check {
